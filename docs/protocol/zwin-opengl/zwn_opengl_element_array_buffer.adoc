= zwn_opengl_element_array_buffer

== destroy
`request`

Destroy a zwn_opengl_element_array_buffer object.
...
zwn_opengl_element_array_buffer.attach assigns the given buffer as the pending
buffer. zwn_virtual_object.commit makes the pending buffer the new element array
...
zwn_opengl_element_array_buffer, the delivery of wl_buffer.release events
...
to more than one zwn_opengl_component.
